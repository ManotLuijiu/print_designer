...
def process_payment_entry_wht(doc, method=None):
    ...
    company_wht_enabled = frappe.db.get_value("Company", doc.company, "thailand_service_business")
    ...
    try:
        ...
        frappe.msgprint(_("Thailand withholding tax applied successfully. WHT Amount: {0}").format(
...
def _calculate_wht_amounts(doc):
    ...
    if not doc.wht_account:
        doc.wht_account = frappe.db.get_value("Company", doc.company, "default_wht_account")
...
def _create_wht_accounting_entries(doc):
    ...
    journal_entry.user_remark = _("Withholding Tax for Payment Entry {0}").format(doc.name)
    ...
    for ref in doc.references:
        if ref.reference_doctype == "Sales Invoice":
            receivable_account = frappe.db.get_value("Sales Invoice", ref.reference_name, "debit_to")
            break
    ...
    doc.add_comment("Comment", _("Withholding Tax Journal Entry created: {0}").format(journal_entry.name))
...
def _update_payment_amounts(doc):
    ...
    doc.add_comment("Comment", _(
        "Payment adjusted for Thailand WHT: Original {0}, WHT {1}, Net {2}"
    ).format(
...
def validate_payment_entry_wht(doc, method=None):
    ...
    if not has_wht_references:
        frappe.msgprint(_("Warning: No sales invoices in this payment are marked as subject to withholding tax"))
    ...
    for ref in doc.references:
        if ref.reference_doctype == "Sales Invoice":
            ...
            if invoice.get("subject_to_wht"):
                ...
                invoice.add_comment("Comment", _(
                    "Withholding Tax applied in Payment {0}: {1} ({2}%)"
                ).format(
                    doc.name,
                    frappe.format_value(doc.wht_amount, {"fieldtype": "Currency"}),
                    doc.wht_rate
...
@frappe.whitelist()
def calculate_estimated_wht(base_amount, wht_rate=3.0):
...
def check_invoice_wht_status(reference_type, reference_name):
    ...
    try:
        if reference_type == "Sales Invoice":
            return frappe.db.get_value("Sales Invoice", reference_name, "subject_to_wht")
        return False
    except:
        return False