...
def get_signature_field_options_string():
    ...
    try:
        ...
        for option in options:
            option_strings.append(f"{option['value']}|{option['label']}")
...
def parse_signature_field_mapping(field_mapping):
    ...
    try:
        if not field_mapping:
            return {"error": "Field mapping is empty"}

        # Handle new format "value|label"
        if "|" in field_mapping:
            field_mapping = field_mapping.split("|")[0]

        if "::" not in field_mapping:
            return {
                "error": "Invalid field mapping format. Expected 'DocType::fieldname'"
            }
...
def get_signature_field_info(field_mapping):
    ...
    try:
        ...
        field_installed = frappe.db.exists(
