...
def get_pdf_with_weasyprint(html, options=None):
    ...
    try:
        ...
        # Get orientation from options, or fallback to Print Settings (convert to lowercase for WeasyPrint)
        page_orientation = options.get("orientation")
        if not page_orientation:
            page_orientation = (
                frappe.db.get_single_value("Print Settings", "page_orientation") or "Portrait"
            )
        orientation = page_orientation.lower()  # WeasyPrint uses lowercase
...
def clean_css_for_weasyprint(css_content):
    ...
    css_content = re.sub(
        r"user-select\s*:\s*[^;]+;?", "", css_content, flags=re.IGNORECASE | re.MULTILINE
    )
