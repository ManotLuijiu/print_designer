...
print_designer.watermark = {
  ...
  inject_sidebar_tooltips: function () {
    const tooltip_map = {
      "Watermark per Page": __(
        "None=off | Original on First Page=first page only | Copy on All Pages=every page | Original,Copy on Sequence=alternating per page",
      ),
    };
  ...
  load_watermark_config: function () {
    frappe.call({
      method:
        "print_designer.api.watermark.get_watermark_config_for_print_format",
      args: {
  ...
  bind_sidebar_events: function () {
    ...
    $(
      ".watermark-position-select, .watermark-margin-top, .watermark-margin-right, .watermark-margin-bottom, .watermark-margin-left, .watermark-font-size, .watermark-color, .watermark-custom-text",
    ).on("change input", () => {
  ...
  update_template_selector: function () {
    ...
    selector
      .empty()
      .append('<option value="">' + __("No Watermark") + "</option>");
  ...
  populate_current_config: function () {
    ...
    $(".watermark-template-select").val(
      this.current_config.template_name || "",
    );
    $(".watermark-mode-select").val(
      this.current_config.watermark_mode || "None",
    );
    $(".watermark-position-select").val(
      this.current_config.position || "Top Right",
    );
  ...
  update_config_from_controls: function () {
    ...
    this.current_config.margin_top =
      parseInt($(".watermark-margin-top").val()) || 0;
    this.current_config.margin_right =
      parseInt($(".watermark-margin-right").val()) || 0;
    this.current_config.margin_bottom =
      parseInt($(".watermark-margin-bottom").val()) || 0;
    this.current_config.margin_left =
      parseInt($(".watermark-margin-left").val()) || 0;
    this.current_config.font_size =
      parseInt($(".watermark-font-size").val()) || 24;
    this.current_config.color = $(".watermark-color").val();
    this.current_config.opacity =
      parseFloat($(".watermark-opacity").val()) || 0.6;
  ...
  apply_watermark: function () {
    ...
    const print_area = $(
      ".print-format-container, .print-preview-wrapper",
    ).first();
    ...
    if (mode === "Copy on All Pages") {
      ...
      if (pages.length > 1) {
        pages.each((index, page) => {
          if (index > 0) {
            // Skip first page as it already has watermark
...
(function () {
  ...
  frappe.ui.form.Form.prototype.print_doc = function () {
    ...
    frappe
      .xcall("print_designer.overrides.print_button.get_print_config", {
      ...
      .then((config) => {
        if (config.raw_printing && config.tbs_agent_available) {
          ...
          // Chrome CDP → open print preview page (user can then print or download PDF)
          // Don't pass _lang - let print preview page determine language from Print Format's default_print_language
          const preview_url = `/desk/print/${encodeURIComponent(this.doctype)}/${encodeURIComponent(this.doc.name)}?format=${encodeURIComponent(print_format)}`;
   ...
   */
  frappe.ui.form.Form.prototype._submit_raw_print_via_tbs = function (
    print_format,
    config,
  ) {
    frappe.show_alert({
    ...
    frappe
      .xcall("print_designer.overrides.print_button.submit_raw_print", {
        doc: this.doctype,
        name: this.doc.name,
        print_format: print_format,
  ...
  const _patch_print_view = function () {
     ...
     */
    frappe.ui.form.PrintView.prototype._tbs_raw_print = function (
      print_format,
      config,
    ) {
      frappe.show_alert({
      ...
      frappe
        ...
        .catch((err) => {
          frappe.msgprint({
            title: __("Print Error"),
            message:
              err.message || __("Failed to submit print job to TBS agent"),
            indicator: "red",
          });
        });
    };
  ...
  if (frappe.ui.form.PrintView) {
    ...
    $(document).on("page-change", function () {
      if (
        ...
        !frappe.ui.form.PrintView.prototype._tbs_raw_print
      ) {
        _patch_print_view();
      }
    });
