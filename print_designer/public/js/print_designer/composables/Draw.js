...
export function useDraw() {
  ...
  const handleScroll = (_e) => {
    parameters.scrollX =
      canvas.parentElement.scrollLeft - parameters.initialScrollX;
    parameters.scrollY =
      canvas.parentElement.scrollTop - parameters.initialScrollY;
  };
  const drawEventHandler = {
    ...
    mouseup: (_e) => {
      ...
      // NOTE: minRows recalculation from height is handled EXCLUSIVELY by Resizable.js
      // (resizeend handler). Draw.js does NOT update minRows — Resize.js handles it
      // when the element is dropped onto the canvas.
    },
  };
