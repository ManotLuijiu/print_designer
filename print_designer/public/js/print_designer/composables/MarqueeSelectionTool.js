...
export function useMarqueeSelection() {
  ...
  function mouseDown(e) {
    ...
    marqueeElement.style.left =
      parameters.startX - canvas.getBoundingClientRect().left + "px";
    marqueeElement.style.top =
      parameters.startY - canvas.getBoundingClientRect().top + "px";
  }
  ...
  function mouseUp(e) {
    ...
    if (marqueeElement) {
      ...
      // Use a distinct name to avoid shadowing the directive `canvas`
      // (which is the DOM node used by mouseUp/mouseDown).
      ...
      for (const page of ElementStore.Elements) {
        // DOMRef is initialized to null and may be missing during
        // initialization, mode changes, or route teardown.
        if (!page?.DOMRef?.getBoundingClientRect) continue;

        const pageRect = page.DOMRef.getBoundingClientRect();
        const pageRelativeBounds = { ...selectionBounds };
        pageRelativeBounds.x -= pageRect.x;
        pageRelativeBounds.y -= pageRect.y;
        for (const element of page.childrens) {
          ...
          if (!element.relativeContainer && isInBounds(pageRelativeBounds, b)) {
