...
export const setCurrentElement = (event, element) => {
  ...
  if (
    !event.shiftKey &&
    !MainStore.getCurrentElementsValues.includes(element)
  ) {
  ...
  if (
    MainStore.getCurrentElementsId.length < 2 &&
    !MainStore.currentElements[element.id]
  ) {
...
export const recursiveChildrens = ({
  element,
  isClone = false,
  isMainElement = true,
}) => {
  ...
  isMainElement &&
    childrensCleanUp(parentElement, element, isClone, isMainElement);
...
export const updateElementParameters = (e) => {
  ...
  if (restrict && !e.metaKey && !e.ctrlKey) {
    if (parameters.isReversedY) {
      ...
      if (
        restrict.bottom &&
        restrict.bottom - parameters.startY < parameters.height
      ) {
        MainStore.lastCreatedElement.height = Math.abs(
    ...
    if (parameters.isReversedX) {
      ...
      if (
        restrict.right &&
        restrict.right - parameters.startX < parameters.width
      ) {
        MainStore.lastCreatedElement.width = Math.abs(
          restrict.right - parameters.startX,
        );
      } else {
        MainStore.lastCreatedElement.startX = parameters.startX;
        MainStore.lastCreatedElement.width = parameters.width;
  ...
  if (field.value != "" && field.parseJinja) {
    try {
      ...
      if (result.success) {
        ...
        console.error(
          "Error From User Provided Jinja String\n\n",
          result.error,
        );
      }
    } catch (error) {
  ...
  if (icon == "borderAll") {
    if (
      [
        ...
        "borderLeftStyle",
        "borderRightStyle",
      ].every((side) => element[side] != "hidden")
    ) {
...
const getGlobalStyleObject = (object = null, checkProperty = null) => {
  ...
  switch (MainStore.globalStyles[globalStyleName].styleEditMode) {
    ...
    case "alt":
      if (
        checkProperty &&
        MainStore.globalStyles[globalStyleName].altStyle[checkProperty]
      ) {
    ...
    if (object) {
      switch (object.styleEditMode) {
        ...
        case "label":
          if (field.isFontStyle) {
            object =
              object.selectedDynamicText?.labelStyle || object.labelStyle;
          } else {
        ...
        case "header":
          if (field.isFontStyle) {
            object =
              object.selectedDynamicText?.headerStyle || object.headerStyle;
          } else {
...
export const handlePrintFonts = (element, printFonts) => {
  ...
  const pushFonts = ({ el = null, styleEditMode, globalStyleName }) => {
    ...
    if (
      printFonts[fontFamily][weightArray].indexOf(parseInt(fontWeight)) == -1
    ) {
