...
class PDFGenerationLogger {
  ...
  logPDFFreeze(url, generator, format, timeoutDuration) {
    return this.log(
      "PDF_GENERATION_FREEZE",
      "PDF generation appears to be frozen",
      {
        url,
        generator,
        format,
  ...
  logPDFRetry(url, newGenerator, previousGenerator, retryCount) {
    ...
    return this.log(
      ...
      `Retrying PDF generation with ${newGenerator}`,
      {
        url,
        newGenerator,
        previousGenerator,
  ...
  checkPDFSupport() {
    return {
      ...
      pdfViewerSupported:
        "PDFViewer" in window ||
        ("chrome" in window && "webstore" in window.chrome),
    };
  }
  ...
  exportSessionLogs() {
    const sessionLogs = this.logQueue.filter(
...
if (!window.pdfLogger) {
  ...
  window.pdfLogger.log(
    "LOGGER_INITIALIZED",
    "PDF Logger initialized successfully",
    {
      loggerVersion: "1.0",
        ...
        if (document.readyState === "loading") {
          document.addEventListener("DOMContentLoaded", () => {
            try {
              ...
              console.error(
                "Error initializing print page on DOM load:",
                error,
              );
            }
          });
  ...
  function extendPrintView() {
    ...
    frappe.ui.form.PrintView = class PrintView extends OriginalPrintView {
      ...
      setup_copy_controls() {
        ...
        this.copy_count_input =
          this.copy_controls_container.find(".copy-count-input");
        this.copy_decrease_btn =
          this.copy_controls_container.find(".copy-decrease");
        this.copy_increase_btn =
          this.copy_controls_container.find(".copy-increase");
        this.original_label_input = this.copy_controls_container.find(
          ".original-label-input",
        );
        this.copy_label_input =
          this.copy_controls_container.find(".copy-label-input");
      ...
      sync_copy_controls_with_sidebar() {
        ...
        if (this.copy_labels_item) {
          const originalLabel =
            this.original_label_input.val() || __("Original");
      ...
      showDownloadFallback(url, wrapperContainer, canvasContainer) {
        ...
        if (window.pdfLogger) {
          window.pdfLogger.log(
            "PDF_GENERATION_FINAL_FAILURE",
            "All PDF generation and display attempts failed",
            {
              url: url,
        ...
        frappe.show_alert(
          {
            message: __(
              "PDF cannot be displayed inline. Please download the PDF to view it, or try using a different PDF generator from the sidebar.",
            ),
            indicator: "orange",
          },
          8,
      ...
      async designer_pdf(print_format) {
        ...
        if (
          this.watermark_selector &&
          this.watermark_selector.val() &&
          this.watermark_selector.val() !== "None"
        ) {
          const watermarkValue = this.watermark_selector.val();
          console.log(
            "[WATERMARK DEBUG] Adding watermark to Print Designer PDF preview:",
            watermarkValue,
          );
          ...
          if (watermarkValue.startsWith("Template: ")) {
            ...
            console.log(
              "[WATERMARK DEBUG] Using watermark template:",
              templateName,
            );
          } else {
            params.set("watermark_settings", watermarkValue);
            console.log(
              "[WATERMARK DEBUG] Using basic watermark mode:",
              watermarkValue,
            );
          }
        }
        ...
        if (window.safePDFClient) {
          try {
            ...
            if (window.pdfLogger) {
              window.pdfLogger.log(
                "PDF_SAFE_CLIENT_USED",
                "Using safe PDF client for URL generation",
                {
            ...
            console.error(
              "Print Designer: Safe PDF client failed, using standard URL:",
              error,
            );
            ...
            if (window.pdfLogger) {
              window.pdfLogger.log(
                "PDF_SAFE_CLIENT_FAILED",
                "Safe PDF client failed, using standard URL",
                {
                  error: error.message,
                  fallbackUrl: url,
                },
    ...
    `;
        ...
        if (window.pdfLogger) {
          ...
          window.pdfLogger.log(
            "PDF_LANGUAGE_INFO",
            "PDF generation language selection",
            {
              ...
              storedLanguage:
                localStorage.getItem("print_designer_language") || "not set",
            },
            "INFO",
          );
          ...
          if (params.get("digital_signature") || params.get("company_stamp")) {
            window.pdfLogger.log(
              "PDF_SIGNATURE_STAMP_INFO",
              "PDF generation with signature/stamp",
              {
                digitalSignature:
                  params.get("digital_signature") || "not selected",
          ...
          this.checkPDFUrl(url)
            .then(async (urlCheck) => {
              ...
              if (urlCheck.isPermissionError) {
                ...
                if (window.pdfLogger) {
                  window.pdfLogger.log(
                    "PDF_PERMISSION_ERROR",
                    "PDF generation failed due to permission error",
                    {
                      url: url,
                ...
                if (
                  this.letterhead_selector &&
                  this.letterhead_selector.val() &&
                  !this.letterhead_retry_attempted
                ) {
                  ...
                  frappe.show_alert(
                    {
                      message: __(
                        "Permission denied with letterhead. Retrying without letterhead...",
                      ),
                      indicator: "orange",
                    },
                    3,
                  ...
                  if (
                    this.enable_copies_item &&
                    this.enable_copies_item.value
                  ) {
                    retryParams.set(
                      "copy_count",
                      this.copy_count_item.value || 2,
                    );
                    if (this.copy_labels_item.value) {
                      retryParams.set(
                        "copy_labels",
                        this.copy_labels_item.value,
                      );
                    }
                  ...
                  if (
                    this.signature_selector &&
                    this.signature_selector.val()
                  ) {
                    retryParams.set(
                      "digital_signature",
                      this.signature_selector.val(),
                  ...
                  if (
                    this.company_stamp_selector &&
                    this.company_stamp_selector.val()
                  ) {
                    retryParams.set(
                      "company_stamp",
                      this.company_stamp_selector.val(),
                  ...
                  if (window.safePDFClient) {
                    try {
                      ...
                      retryUrl =
                        await window.safePDFClient.getPDFDownloadURL(
                          retryParamsObj,
                        );
                    } catch (error) {
                  ...
                  if (window.pdfLogger) {
                    window.pdfLogger.log(
                      "PDF_LETTERHEAD_RETRY",
                      "Retrying PDF generation without letterhead due to permission error",
                      {
                        originalUrl: url,
                        retryUrl: retryUrl,
                  ...
                  setTimeout(() => {
                    ...
                    const existingRetryEl = document.getElementById(
                      "pd-pdf-viewer-retry",
                    );
                    ...
                    console.log(
                      "Letterhead present in retry URL:",
                      retryUrl.includes("letterhead"),
                    );
                    ...
                    newPdfEl.addEventListener("load", () => {
                      ...
                      frappe.show_alert(
                        {
                          message: __(
                            "PDF generated successfully without letterhead",
                          ),
                          indicator: "green",
                        },
                        3,
                    ...
                    newPdfEl.addEventListener("error", () => {
                      ...
                      const handleRetryError = () => {
                        ...
                        this.checkPDFUrl(retryUrl)
                          .then((urlCheck) => {
                            if (window.pdfLogger) {
                              window.pdfLogger.log(
                                "PDF_LETTERHEAD_RETRY_FAILED",
                                "PDF generation failed even without letterhead",
                                {
                                  retryUrl: retryUrl,
                                  originalLetterhead: originalLetterhead,
                                  generator: selected_generator,
                            ...
                            if (urlCheck.isPermissionError) {
                              errorMessage = __(
                                "PDF generation failed: Access denied even without letterhead. This may be a deeper permissions issue.",
                              );
                            } else if (urlCheck.isServerError) {
                              errorMessage = __(
                                "PDF generation failed: Server error ({0}). Please try again later.",
                                [urlCheck.status],
                              );
                            } else if (urlCheck.isNetworkError) {
                              errorMessage = __(
                                "PDF generation failed: Network error. Please check your connection.",
                              );
                            } else {
                              errorMessage = __(
                                "PDF generation failed even without letterhead. This may be a server or permissions issue.",
                              );
                            }
                ...
                let errorMessage = __(
                  "PDF generation failed: Access denied (403)",
                );
                if (
                  this.letterhead_selector &&
                  this.letterhead_selector.val()
                  ...
                  errorMessage +=
                    ". " +
                    __(
                      "This might be due to letterhead permissions. Try generating without letterhead.",
                    );
                }
              ...
              if (urlCheck.isNetworkError) {
                ...
                if (window.pdfLogger) {
                  window.pdfLogger.log(
                    "PDF_NETWORK_ERROR",
                    "PDF generation failed due to network error",
                    {
                      url: url,
              ...
              const alternativeGenerators = ["wkhtmltopdf", "chrome"].filter(
                (g) => g !== currentGenerator,
              );

              if (
                alternativeGenerators.length > 0 &&
                !this.pdf_retry_attempted &&
                !urlCheck.isClientError
              ) {
                ...
                frappe.show_alert(
                  {
                    message: __("Retrying with {0} generator...", [
                      nextGenerator,
                    ]),
                    indicator: "blue",
                  },
              ...
              if (!this.iframe_fallback_attempted) {
                ...
                if (window.pdfLogger) {
                  window.pdfLogger.log(
                    "PDF_FALLBACK_IFRAME",
                    "Attempting iframe fallback for PDF display",
                    {
                      url: url,
        ...
        const resetFreezeTimeout = () => {
          ...
          freezeTimeout = setTimeout(() => {
            ...
            frappe.show_alert(
              {
                message: __(
                  "PDF generation appears to be stuck. This may be due to server overload or network issues.",
                ),
                indicator: "orange",
              },
              10,
            ...
            setTimeout(() => {
              if (
                confirm(
                  __("PDF generation seems frozen. Would you like to retry?"),
                )
              ) {
                location.reload();
              }
      ...
      printit() {
        ...
        if (this.enable_copies_item && this.enable_copies_item.value) {
          ...
          frappe.show_alert({
            message: __(
              "Multiple copies detected. Downloading PDF for printing...",
            ),
            indicator: "blue",
          });
          this.render_pdf();
      ...
      printit_print_designer() {
        ...
        const copyCount = this.copy_count_input
          ? parseInt(this.copy_count_input.val()) || 1
          : this.copy_count_item
            ? parseInt(this.copy_count_item.value) || 1
            : 1;

        if (
          copyCount > 1 ||
          (this.enable_copies_item && this.enable_copies_item.value)
        ) {
        ...
        if (
          ...
          const watermarkValue = this.watermark_selector.val();
          console.log(
            "[WATERMARK DEBUG] Adding watermark to print:",
            watermarkValue,
          );
          ...
          if (watermarkValue.startsWith("Template: ")) {
            ...
            params.set("watermark_template", templateName);
            console.log(
              "[WATERMARK DEBUG] Using watermark template for print:",
              templateName,
            );
          } else {
            params.set("watermark_settings", watermarkValue);
            console.log(
              "[WATERMARK DEBUG] Using basic watermark mode for print:",
              watermarkValue,
            );
          }
        }
      ...
      preview() {
        ...
        console.log(
          "[WATERMARK DEBUG] Has print_designer:",
          !!print_format?.print_designer,
        );
        console.log(
          "[WATERMARK DEBUG] Has print_designer_body:",
          !!print_format?.print_designer_body,
        );
        console.log(
          "[WATERMARK DEBUG] sidebar_dynamic_section exists:",
          !!this.sidebar_dynamic_section,
        );
        console.log(
          "[WATERMARK DEBUG] watermark_selector exists:",
          !!this.watermark_selector,
        );
        ...
        if (print_format.print_designer && print_format.print_designer_body) {
          ...
          this.page.add_menu_item("Export PDF Logs", () =>
            this.exportPDFLogs(),
          );
          ...
          console.log(
            "[WATERMARK DEBUG] About to show sidebar_dynamic_section...",
          );
          if (this.sidebar_dynamic_section) {
            this.sidebar_dynamic_section.show();
            console.log(
              "[WATERMARK DEBUG] sidebar_dynamic_section.show() called",
            );
            console.log(
              "[WATERMARK DEBUG] sidebar_dynamic_section is visible:",
              this.sidebar_dynamic_section.is(":visible"),
            );
            console.log(
              "[WATERMARK DEBUG] sidebar_dynamic_section display style:",
              this.sidebar_dynamic_section.css("display"),
            );
          } else {
            console.log(
              "[WATERMARK DEBUG] ERROR: sidebar_dynamic_section is null/undefined!",
            );
          }
          ...
          if (this.watermark_selector) {
            console.log(
              "[WATERMARK DEBUG] watermark_selector exists, parent visible:",
              this.watermark_selector.parent().is(":visible"),
            );
            console.log(
              "[WATERMARK DEBUG] watermark_selector display:",
              this.watermark_selector.css("display"),
            );
            console.log(
              "[WATERMARK DEBUG] watermark_selector parent classes:",
              this.watermark_selector.parent().attr("class"),
            );
          } else {
            console.log(
              "[WATERMARK DEBUG] ERROR: watermark_selector is null/undefined!",
            );
          }
        ...
        console.log(
          "[WATERMARK DEBUG] About to show sidebar_dynamic_section for traditional format...",
        );
        if (this.sidebar_dynamic_section) {
          this.sidebar_dynamic_section.show();
          console.log(
            "[WATERMARK DEBUG] sidebar_dynamic_section.show() called for traditional",
          );
          console.log(
            "[WATERMARK DEBUG] sidebar_dynamic_section is visible:",
            this.sidebar_dynamic_section.is(":visible"),
          ...
          console.log(
            "[WATERMARK DEBUG] ERROR: sidebar_dynamic_section is null/undefined for traditional!",
          );
        }
      ...
      setup_sidebar() {
        ...
        this.print_format_item = this.add_sidebar_item({
          ...
          change: () => {
            if (
              this.print_format_item.value == this.print_format_item.last_value
            )
              return;
            this.toolbar_print_format_selector.set_value(
              this.print_format_item.value,
            );
            // Update language when print format changes
            this.set_default_print_language();
            this.refresh_print_format();
          },
        });
      ...
      `).appendTo(this.sidebar);
          ...
          this.pdf_generator_item = this.add_sidebar_item({
            ...
            options: ["auto", "wkhtmltopdf", "WeasyPrint", "chrome"].join("\n"),
            default: "auto",
            description: __(
              "Auto lets server choose the best available generator",
            ),
            change: () => {
          ...
          if (
            this.pdf_generator_item &&
            this.pdf_generator_item.df.options.indexOf("WeasyPrint") === -1
          ) {
            this.pdf_generator_item.df.options =
              this.pdf_generator_item.df.options.replace(
                "chrome",
                "WeasyPrint\nchrome",
              );
      ...
      load_print_settings() {
        ...
        Promise.all([
          ...
          new Promise((resolve) => {
            frappe.call({
              ...
              callback: (r) => {
                if (r.message) {
                  console.log(
                    "[WATERMARK DEBUG] Print settings loaded:",
                    r.message,
                  );
                  resolve(r.message);
                } else {
                  console.log(
                    "[WATERMARK DEBUG] No print settings returned from API",
                  );
              ...
              error: (err) => {
                console.log(
                  "[WATERMARK DEBUG] Error loading print settings:",
                  err,
                );
          ...
          new Promise((resolve) => {
            frappe.call({
              method:
                "print_designer.api.watermark.get_available_watermark_templates",
              callback: (r) => {
                if (r.message) {
                  console.log(
                    "[WATERMARK DEBUG] Watermark templates loaded:",
                    r.message,
                  );
                  resolve({ templates: r.message });
                } else {
                  console.log(
                    "[WATERMARK DEBUG] No watermark templates available",
                  );
                  resolve({ templates: [] });
                }
              },
              error: (err) => {
                console.log(
                  "[WATERMARK DEBUG] Error loading watermark templates:",
                  err,
                );
                resolve({ templates: [] });
              },
      ...
      setup_copy_options() {
        ...
        console.log(
          "[WATERMARK DEBUG] copy_options_initialized:",
          this.copy_options_initialized,
        );
        console.log(
          "[WATERMARK DEBUG] print_settings exists:",
          !!this.print_settings,
        );
        ...
        if (this.copy_options_initialized || !this.print_settings) {
          console.log(
            "[WATERMARK DEBUG] Early return from setup_copy_options - already initialized or no settings",
          );
          return;
        }
        ...
        console.log(
          "[WATERMARK DEBUG] Setting copy_options_initialized = true",
        );

        // Create enhanced watermark selector with templates integration
        console.log(
          "[WATERMARK DEBUG] Creating enhanced watermark selector...",
        );
        console.log(
          "[WATERMARK DEBUG] Available watermark templates:",
          this.watermark_templates,
        );
        ...
        try {
          ...
          this.watermark_selector = this.add_sidebar_item({
            ...
            description: __(
              "Watermark options: Basic modes (None, Original, Copy, Sequence) or custom templates from Watermark Settings",
            ),
            change: () => {
              const selectedValue = this.watermark_selector.val();
              console.log(
                "[WATERMARK DEBUG] Watermark selector changed, value:",
                selectedValue,
              );
          ...
          console.log(
            "[WATERMARK DEBUG] Enhanced watermark selector created:",
            this.watermark_selector,
          );
          ...
          if (this.sidebar_dynamic_section && this.watermark_selector) {
            console.log(
              "[WATERMARK DEBUG] Moving watermark selector to sidebar_dynamic_section...",
            );
            this.watermark_selector
              .parent()
              .appendTo(this.sidebar_dynamic_section);
            console.log(
              "[WATERMARK DEBUG] Watermark selector moved to dynamic section",
            );
          }
        } catch (error) {
          console.log(
            "[WATERMARK DEBUG] ERROR creating enhanced watermark selector:",
            error,
          );
        }
        ...
        if (this.print_settings.show_copy_controls_in_toolbar) {
          console.log(
            "[WATERMARK DEBUG] Copy controls enabled in toolbar - watermark should be visible",
          );
        } else {
          console.log(
            "[WATERMARK DEBUG] Copy controls NOT enabled in toolbar - but showing anyway for debug",
          );
        }
      ...
      `).appendTo(this.sidebar);
          ...
          this.copy_labels_item = this.add_sidebar_item({
            ...
            placeholder:
              (this.print_settings.default_original_label || __("Original")) +
              ", " +
            ...
            change: () => {
              ...
              if (this.original_label_input && this.copy_label_input) {
                const labels = (this.copy_labels_item.value || "").split(",");
                const originalLabel = labels[0]
                  ? labels[0].trim()
                  : this.print_settings.default_original_label ||
                    __("Original");
                const copyLabel = labels[1]
                  ? labels[1].trim()
                  : this.print_settings.default_copy_label || __("Copy");
      ...
      show_watermark_template_info(templateName) {
        ...
        const template = this.watermark_templates.find(
      ...
      refresh_copy_options_labels() {
        ...
        if (this.copy_count_item) {
          ...
          this.copy_count_item.df.description = __(
            "Total number of copies to generate",
          );
        ...
        if (this.copy_labels_item) {
          ...
          this.copy_labels_item.df.placeholder =
            __("Original, Copy") + " (" + __("Optional") + ")";
          this.copy_labels_item.df.description = __(
            "Comma-separated labels for each copy",
          );
      ...
      set_default_signature_and_stamp() {
        let company =
          this.frm.doc.company || frappe.defaults.get_user_default("Company");

        if (company && !this.signature_selector.val()) {
          // Try to set default signature for company
          frappe.db
            .get_list("Digital Signature", {
      ...
      set_default_print_language() {
        ...
        // 2. User's explicit language selection (stored in localStorage)
        // 3. Document's language field
        // 4. User's GUI language (frappe.boot.lang)
        // 5. Thai as final fallback

        console.log(
          "[Language Debug] ====== START set_default_print_language ======",
        );

        let print_format_name = this.selected_format();
        console.log(
          "[Language Debug] selected_format() returned:",
          print_format_name,
        );
        console.log(
          "[Language Debug] print_format_item.value:",
          this.print_format_item?.value,
        );
        console.log(
          "[Language Debug] toolbar_print_format_selector.value:",
          this.toolbar_print_format_selector?.value,
        );

        // Also check if there's a different source
        if (this.frm?.meta?.default_print_format) {
          console.log(
            "[Language Debug] frm.meta.default_print_format:",
            this.frm.meta.default_print_format,
          );
        }

        // Check the Print Format doctype for the current doc
        if (this.frm?.doc?.print_format) {
          console.log(
            "[Language Debug] frm.doc.print_format:",
            this.frm.doc.print_format,
          );
        }

        // BUG FIX: Ensure we always get the full Print Format document with default_print_language
          ...
          frappe.call({
            ...
            callback: (r) => {
              ...
              if (r.message) {
                ...
                console.log(
                  "[Language Debug] print_format.default_print_language:",
        ...
        // Debug logging
        console.log(
          "[Language Debug] Final print_format_name:",
          print_format_name,
        );
        console.log(
          "[Language Debug] Print format default_print_language:",
          print_format?.default_print_language,
        );
        console.log(
          "[Language Debug] Document language:",
          this.frm?.doc?.language,
        );
        console.log("[Language Debug] frappe.boot.lang:", frappe.boot.lang);
        console.log(
          "[Language Debug] LocalStorage language:",
          localStorage.getItem("print_designer_language"),
        );

        // Priority 1: Print Format's default_print_language
        if (print_format?.default_print_language) {
          this.lang_code = print_format.default_print_language;
          console.log(
            "[Language Debug] Using print format language:",
            this.lang_code,
          );
        }
        // Priority 2: User's explicit selection (don't override user's choice)
        else if (localStorage.getItem("print_designer_language")) {
          this.lang_code = localStorage.getItem("print_designer_language");
          console.log(
            "[Language Debug] Using localStorage language:",
            this.lang_code,
          );
        }
        // Priority 3: Document's language
        else if (this.frm?.doc?.language) {
          this.lang_code = this.frm.doc.language;
          console.log(
            "[Language Debug] Using document language:",
            this.lang_code,
          );
        }
        // Priority 4: User's GUI language
        else if (frappe.boot.lang) {
          this.lang_code = frappe.boot.lang;
          console.log("[Language Debug] Using GUI language:", this.lang_code);
        }
        // Priority 5: Fallback to Thai
        else {
          this.lang_code = "th";
          console.log("[Language Debug] Using fallback language: th");
        }

        console.log("[Language Debug] Final lang_code:", this.lang_code);
        console.log(
          "[Language Debug] ====== END set_default_print_language ======",
        );
      ...
      update_language_selectors() {
        // Update all language selectors with current lang_code
        if (this.language_selector) {
          this.language_selector.val(this.lang_code);
        }
        if (this.language_item) {
        ...
        if (this.toolbar_language_selector?.$input) {
          this.toolbar_language_selector.$input.val(this.lang_code);
        }
        // Also update the sidebar language selector
        if (this.language_item?.value !== this.lang_code) {
      ...
      async render_pdf() {
        ...
        if (copyCount > 1) {
          ...
          const originalLabel = this.original_label_input
            ? this.original_label_input.val() || __("Original")
            : __("Original");
          const copyLabel = this.copy_label_input
            ? this.copy_label_input.val() || __("Copy")
            : __("Copy");
          params.set("copy_labels", `${originalLabel}, ${copyLabel}`);
        ...
        if (
          ...
          const watermarkValue = this.watermark_selector.val();
          console.log(
            "[WATERMARK DEBUG] Adding watermark parameter to PDF URL:",
            watermarkValue,
          );
          ...
          if (watermarkValue.startsWith("Template: ")) {
            ...
            params.set("watermark_template", templateName);
            console.log(
              "[WATERMARK DEBUG] Using watermark template for PDF download:",
              templateName,
            );
          } else {
            params.set("watermark_settings", watermarkValue);
            console.log(
              "[WATERMARK DEBUG] Using basic watermark mode for PDF download:",
              watermarkValue,
            );
          }
        }
        ...
        if (window.safePDFClient) {
          try {
            ...
            console.error(
              "Print Designer: Safe PDF client failed for copy printing, using standard URL:",
              error,
            );
            url = `${window.location.origin}/api/method/frappe.utils.print_format.download_pdf?${params.toString()}`;
          }
      ...
      async clearPDFLogs() {
        ...
        if (
          confirm(__("Are you sure you want to clear all PDF generation logs?"))
        ) {
          try {
