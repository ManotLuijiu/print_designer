...
export const useElementStore = defineStore("ElementStore", {
  ...
  actions: {
    ...
    async loadElements(printDesignName) {
      ...
      // pushes into. Without this, every reload / save / re-open of this editor
      // would append another copy of the design into `Headers`, `Footers`, and
      // `dynamicData` (the latter is populated transitively by `childrensLoad`
      // for every element below). RAM grew indefinitely with no GC release.
      const MainStore = useMainStore();
      this.Headers.length = 0;
      this.Footers.length = 0;
      ...
      ElementsBody.forEach((page, index) => {
        ...
        // BUG FIX (Footer only): Footer children stick to where they
        // were dragged, even after the user changes footerHeight.
        // When footerHeight is set (e.g. 50mm) the wrapper correctly
        // anchors at page.height - footerHeight, but children keep
        // their old startY values — leaving an empty wrapper at the
        // bottom of the printed page and the actual footer items
        // rendered somewhere in the upper area. Translate every
        // footer child so the entire cluster sits flush against the
        // bottom edge. We deliberately skip the header wrapper —
        // the header lays items out from the top of the page and
        // fixing it here would push items DOWN to the bottom of the
        // header band, which is the opposite of what header sections
        // should do.
        this.alignFooterToBottom(page.footer[0]);
      });
    ...
    alignFooterToBottom(wrapper) {
      ...
      // cluster's bottom edge sits flush against the wrapper's bottom
      // edge. The wrapper itself is anchored at
      // page.height - footerHeight - marginTop - marginBottom by
      // createHeaderFooterElement (and re-affirmed by AppPages'
      // watcher); children's `startY` is **relative to the wrapper**
      // (the wrapper is the only positioned ancestor), so anchoring
      // them to the wrapper bottom means putting the cluster's bottom
      // edge at y=wrapperHeight inside the wrapper's own coordinate
      // system — NOT at y=page.height in the page's coordinate system.
      // Using `wrapper.startY + wrapperHeight` here would translate
      // children by an extra ~page.height-footerHeight px and push
      // them off the page entirely.
      if (!wrapper || !wrapper.childrens || !wrapper.childrens.length) {
        return;
      }
      const wrapperHeight = Number(wrapper.height) || 0;
      if (wrapperHeight <= 0) return;
      const maxChildBottom = Math.max(
        ...wrapper.childrens.map(
          (c) => (Number(c.startY) || 0) + (Number(c.height) || 0),
        ),
      );
      const delta = wrapperHeight - maxChildBottom;
      if (Math.abs(delta) < 0.5) return; // already aligned to bottom
      for (const child of wrapper.childrens) {
        child.startY = (Number(child.startY) || 0) + delta;
        if (typeof child.pageY === "number") {
          child.pageY += delta;
        }
      }
    },
