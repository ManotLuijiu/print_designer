...
export const useMainStore = defineStore("MainStore", {
  state: () => ({
    ...
    visiblePages: [],
    cursor:
      "url('/assets/print_designer/images/mouse-pointer.svg'), default !important",
     ...
     */
    activeToggle: null,
    // Performance optimization: debounce thumbnail generation
    lastSaveTime: 0, // Timestamp of last save
    thumbnailDebounceMs: 5000, // Skip thumbnail if saved within 5 seconds
    isGeneratingThumbnail: false, // Prevent concurrent thumbnail generation
    thumbnailEnabled: true, // Toggle thumbnail generation (can disable for faster saves)
    page: {
      height: 1122.519685,
      width: 793.7007874,
      marginTop: 0,
    ...
    controls: {
      MousePointer: {
        ...
        id: "mouse-pointer",
        cursor:
          "url('/assets/print_designer/images/mouse-pointer.svg'), default",
        isDisabled: false,
      },
      Text: {
        icon: "textTool",
      ...
      Rectangle: {
        ...
        id: "rectangle",
        cursor:
          "url('/assets/print_designer/images/add-rectangle.svg') 6 6, crosshair",
        isDisabled: false,
      },
      Image: {
        icon: "imageTool",
        ...
        id: "image",
        cursor:
          "url('/assets/print_designer/images/add-image.svg') 6 6, crosshair",
        isDisabled: false,
      },
      Table: {
        icon: "tableTool",
        ...
        id: "table",
        cursor:
          "url('/assets/print_designer/images/add-table.svg') 6 6, crosshair",
        isDisabled: false,
      },
      // Components: {
      // 	icon: "fa fa-cube",
      ...
      Barcode: {
        ...
        id: "barcode",
        cursor:
          "url('/assets/print_designer/images/add-barcode.svg') 6 6, crosshair",
      },
    },
    propertiesPanel: [],
  }),
  getters: {
    ...
    getTypeWiseMetaFields: (state) => {
      return ({
        ...
        metaFields.forEach((field) => {
          if (
            ["Button", "Color", "Table", "Attach"].indexOf(field.fieldtype) ==
              -1 &&
            (selectedTable ||
              ["Link", "Image", "Attach", "Attach Image"].indexOf(
                field.fieldtype,
              ) == -1)
          ) {
    ...
    getGoogleFontWeights: (state) => () => {
      ...
      return [
        ...
        (weight) => state.fonts[fontName][0].indexOf(weight.value) != -1,
      );
    },
    getGlobalStyleObject: (state) => {
      ...
      if (object) {
        ...
        styleEditMode =
          mapper[state.globalStyles[globalStyleName].styleEditMode];
      }
    ...
    getCurrentStyle: (state) => (propertyName) => {
      ...
      if (propertyName != "backgroundColor") {
        ...
        if (
          state.isValidValue(object.selectedColumn?.["style"]?.[propertyName])
        ) {
        ...
        if (
          typeof object.selectedDynamicText?.[styleEditMode][propertyName] ==
          "string"
        ) {
          return object.selectedDynamicText?.[styleEditMode][propertyName];
        }
      ...
      Object.entries(this.globalStyles).forEach((element) => {
        ...
        if (!element[1].labelCssRule) {
          ...
          if (labelSelector) {
            ...
            element[1].labelCssRule = markRaw(
              this.screenStyleSheet.cssRules[id],
            );
          }
        }
        if (!element[1].headerCssRule) {
          ...
          if (headerSelector) {
            ...
            element[1].headerCssRule = markRaw(
              this.screenStyleSheet.cssRules[id],
            );
          }
        }
