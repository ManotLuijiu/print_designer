...
function add_delivery_note_buttons(frm) {
    ...
    if (frm.doc.custom_approval_qr_code) {
        frm.add_custom_button(__('Print with QR'), function() {
    ...
    frappe.call({
        ...
        callback: function(r) {
            ...
            if (r.message && r.message.qr_code) {
                ...
                frappe.model.set_value(frm.doctype, frm.docname, 'custom_approval_qr_code', r.message.qr_code);
                frappe.model.set_value(frm.doctype, frm.docname, 'custom_approval_url', r.message.approval_url);
                
                frm.save().then(() => {
                    frappe.msgprint({
                        title: __('QR Code Generated Successfully'),
                        message: __('QR Code has been generated. Customer can scan this code to approve delivery receipt.'),
                        indicator: 'green'
                    });
        ...
        error: function(r) {
            ...
            frappe.msgprint({
                ...
                message: __('An error occurred while generating the QR code: ') + (r.message || 'Unknown error'),
                indicator: 'red'
            });
        }
    });
...
function update_approval_status_indicator(frm) {
    ...
    if (status === 'Approved') {
        ...
        if (frm.doc.custom_customer_approval_date) {
            frm.dashboard.add_comment(__('Approved on: ') + frappe.datetime.str_to_user(frm.doc.custom_customer_approval_date), 'green');
        }
    } else if (status === 'Rejected') {
        frm.dashboard.add_indicator(__('Goods Received: Rejected'), 'red');
        if (frm.doc.custom_rejection_reason) {
            frm.dashboard.add_comment(__('Reason: ') + frm.doc.custom_rejection_reason, 'red');
        }
    } else {
        frm.dashboard.add_indicator(__('Goods Received: Pending'), 'orange');
        if (frm.doc.custom_approval_url) {
            frm.dashboard.add_comment(__('Waiting for customer approval via QR code'), 'orange');
        }
    }
}
...
function add_custom_css_for_qr_fields(frm) {
    ...
    if (frm.doc.custom_approval_qr_code) {
        frm.get_field('custom_approval_qr_code').$wrapper.find('.control-input').css({
            'max-height': '100px',
            'overflow': 'hidden'
        });
    }
...
function add_payment_entry_buttons(frm) {
    ...
    if (frm.doc.docstatus === 1 && frm.doc.custom_is_withholding_tax) {
        frm.add_custom_button(__('Generate WHT Certificate'), function() {
    ...
    if (frm.doc.custom_wht_certificate_generated) {
        frm.add_custom_button(__('Print WHT Certificate'), function() {
...
function calculate_withholding_tax_amount(frm) {
    if (frm.doc.custom_is_withholding_tax && frm.doc.custom_withholding_tax_rate) {
        let base_amount = 0;
        ...
        if (base_amount > 0) {
            const wht_amount = (base_amount * frm.doc.custom_withholding_tax_rate) / 100;
...
function generate_wht_certificate(frm) {
    ...
    if (!frm.doc.custom_supplier_tax_id) {
        frappe.msgprint({
            ...
            message: __('Please enter Supplier Tax ID before generating WHT certificate.'),
            indicator: 'orange'
        });
    ...
    frappe.model.set_value(frm.doctype, frm.docname, 'custom_wht_certificate_generated', 1);
    frm.save();
    
    frappe.msgprint({
        title: __('WHT Certificate Generated'),
        message: __('Thai Withholding Tax Certificate (Form 50 ทวิ) has been generated successfully.'),
        indicator: 'green'
    });
}
...
function update_wht_status_indicator(frm) {
    if (frm.doc.custom_is_withholding_tax) {
        ...
        if (frm.doc.custom_withholding_tax_amount) {
            frm.dashboard.add_comment(__('WHT Amount: ') + format_currency(frm.doc.custom_withholding_tax_amount), 'blue');
        }
    }
}
...
window.addEventListener('error', function(e) {
    if (e.message && e.message.includes('delivery_approval')) {
        ...
        frappe.msgprint({
            ...
            message: __('An error occurred in the delivery approval system. Please refresh the page and try again.'),
            indicator: 'red'
        });
    }
});
...
console.log('Print Designer Delivery Approval & WHT System initialized successfully');