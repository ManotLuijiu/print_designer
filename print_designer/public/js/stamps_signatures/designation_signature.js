	...
	designation_signature: function (frm) {
		if (frm.doc.designation_signature) {
			...
			frappe.msgprint({
				...
				message: __(
					"Would you like to sync this signature to employees with this designation?",
				),
				primary_action: {
					label: __("Sync Now"),
					action: function () {
...
function show_signature_sync_status(frm) {
	...
	frappe.call({
		method:
			"print_designer.api.designation_signature_sync.get_signature_hierarchy",
		args: {
			...
			frappe.call({
				...
				callback: function (r2) {
					...
					let coverage_percent =
						total_employees > 0
							? Math.round((employees_with_signatures / total_employees) * 100)
							: 0;
					...
					let existing_status = field_wrapper.querySelector(
...
function sync_designation_signatures(frm) {
	...
	frappe.call({
		method:
			"print_designer.api.designation_signature_sync.sync_designation_signatures",
		callback: function (r) {
			if (r.message && r.message.success) {
				frappe.show_alert({
					message: __(
						`Sync completed! Updated ${r.message.designations_updated} designations, ${r.message.employees_synced} employees, ${r.message.users_synced} users`,
					),
					indicator: "green",
				});
...
function show_signature_report(frm) {
	frappe.call({
		method:
			"print_designer.api.designation_signature_sync.get_designation_signature_report",
		callback: function (r) {
			if (r.message && !r.message.error) {
				...
				report.designations.forEach((des) => {
					...
					let coverage_color =
						des.coverage_rate > 80
							? "success"
							: des.coverage_rate > 50
								? "warning"
								: "danger";
	...
	let dialog = new frappe.ui.Dialog({
		...
		primary_action: function () {
			...
			if (mappings && mappings.length > 0) {
				frappe.call({
					method:
						"print_designer.api.designation_signature_sync.bulk_update_designation_signatures",
					args: {
					...
					callback: function (r) {
						if (r.message && r.message.success) {
							frappe.show_alert({
								message: __(
									`Bulk update completed! Updated ${r.message.updated_count} designations`,
								),
								indicator: "green",
							});
							dialog.hide();
							...
							frappe.msgprint({
								...
								message:
									r.message?.errors?.join("<br>") || "Unknown error occurred",
							});
						}
					},
				});
