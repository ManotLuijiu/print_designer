...
def safe_download_pdf(
    ...
    **kwargs,
    ...
    try:
        ...
        allowed_params = {"style", "as_pdf", "output", "password", "pdf_options"}
        filtered_kwargs = {k: v for k, v in kwargs.items() if k in allowed_params}

        # Handle language parameter - PRIORITY: Print Format's default_print_language ALWAYS takes precedence
        # This ensures Print Format language is used even if frontend sends _lang parameter
        if format:
            pf_lang = frappe.db.get_value("Print Format", format, "default_print_language")
            if pf_lang:
                language = pf_lang
        elif "_lang" in kwargs:
        ...
        if any(
            ...
            for pattern in ["add_comment_info", "Unknown column", "erpnext_thailand"]
        ):
...
def safe_get_print_html(
    ...
    **kwargs,
    ...
    try:
        # Filter out parameters that are not expected by safe_get_print
        allowed_params = {"output", "password", "pdf_options"}
        filtered_kwargs = {k: v for k, v in kwargs.items() if k in allowed_params}

        # Handle language parameter - PRIORITY: Print Format's default_print_language ALWAYS takes precedence
        # This ensures Print Format language is used even if frontend sends _lang parameter
        if print_format:
            pf_lang = frappe.db.get_value("Print Format", print_format, "default_print_language")
            if pf_lang:
                language = pf_lang
        elif "_lang" in kwargs:
            language = kwargs["_lang"]
...
def check_third_party_conflicts():
    ...
    return {
        ...
        "conflicts": conflicts,
        # ALWAYS recommend safe endpoints so our language override logic is used
        "safe_endpoints_recommended": True,
    }
    ...
    try:
        ...
        try:
            ...
            if hasattr(frappe, "get_print") and frappe.get_print == thailand_print_utils.get_print:
                info["third_party_conflicts"].append(
                    "erpnext_thailand monkey patched frappe.get_print"
                )
        except Exception:
            pass
