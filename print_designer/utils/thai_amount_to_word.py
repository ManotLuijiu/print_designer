...
def is_thai_format(print_format_name=None, doc=None):
    ...
    if print_format_name:
        try:
            ...
            if hasattr(print_format, 'default_language') and print_format.default_language == "th":
                return True
...
@frappe.whitelist()
def get_amount_in_words(amount, currency="THB", print_format=None, doctype=None, docname=None):
    """
    ...
    if is_thai_format(print_format_name, doc):
        try:
            ...
            if frappe.conf.developer_mode:
                frappe.logger().info(f"Enhanced Thai amount in words for {doc.doctype} {doc.name}: {doc.in_words}")
        except Exception as e:
            frappe.log_error(
