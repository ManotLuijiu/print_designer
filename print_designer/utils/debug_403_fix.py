...
def test_403_fix_endpoint():
    ...
    result = {"status": "success", "message": "403 fix test endpoint working", "tests": []}
        ...
        if hasattr(frappe, "get_print") and frappe.get_print == thailand_print_utils.get_print:
            ...
            try:
                field_exists = frappe.db.get_value(
                    "Custom Field", {"dt": "Print Format", "fieldname": "add_comment_info"}, "name"
                )
                if field_exists:
                    result["tests"].append(
                        {
        ...
        if sales_invoice:
            ...
            try:
                ...
                html_result = safe_get_print_html(
                    doctype="Sales Invoice", name=sales_invoice, print_format="Standard"
                )
    ...
    # Summary
    has_conflicts = any(
        test.get("status") == "detected"
        for test in result["tests"]
        if test["name"] == "ERPNext Thailand monkey patching"
    )
    has_safe_endpoints = any(
        test.get("status") == "available"
        for test in result["tests"]
        if test["name"] == "Safe PDF endpoints"
    )

    if has_conflicts and has_safe_endpoints:
        result["conclusion"] = (
            "Conflicts detected but safe endpoints available - 403 errors should be resolved"
        )
    elif has_conflicts and not has_safe_endpoints:
        result["conclusion"] = (
            "Conflicts detected but safe endpoints not available - 403 errors may occur"
        )
...
def test_user_specific_403():
    ...
    TEST_DOCNAME = "TEST-2025-00001"
    TEST_FORMAT = "Your Print Format"

    result = {
        "status": "success",
        "message": "Testing user's specific 403 error scenario",
        "url": f"{SITE_URL}/api/method/frappe.utils.print_format.download_pdf",
        "params": {
    ...
    try:
        doc_exists = frappe.db.exists(TEST_DOCTYPE, TEST_DOCNAME)
        result["tests"].append(
            {
                "name": "Document exists",
                "status": "exists" if doc_exists else "not_found",
                "message": f"Document {TEST_DOCNAME} {'exists' if doc_exists else 'not found'}",
            }
        )
    except Exception as e:
        result["tests"].append(
    ...
    try:
        pf_exists = frappe.db.exists("Print Format", TEST_FORMAT)
        result["tests"].append(
            {
                "name": "Print format exists",
                "status": "exists" if pf_exists else "not_found",
                "message": f"Print format '{TEST_FORMAT}' {'exists' if pf_exists else 'not found'}",
            }
        )
    except Exception as e:
        result["tests"].append(
    ...
    try:
        result_pdf = frappe.get_print(
            doctype=TEST_DOCTYPE, name=TEST_DOCNAME, print_format=TEST_FORMAT, as_pdf=True
        )
    ...
    try:
        ...
        safe_result = safe_get_print_html(
            doctype=TEST_DOCTYPE, name=TEST_DOCNAME, print_format=TEST_FORMAT
        )
