...
class Browser:
    def __init__(self, generator, print_format, html, options):
        self.is_print_designer = frappe.get_cached_value(
        ...
        self.body_pdf = self.body_page.generate_pdf(
            raw=not self.header_page and not self.footer_page
        )
    ...
    def set_options(self, options):
        ...
        print(f"self.copy_count {self.copy_count}")

        self.copy_labels = (
            self.options.get("copy_labels", [frappe._("Original"), frappe._("Copy")])
            if self.options
    ...
    def prepare_options_for_pdf(self):
        ...
        updated_options["paperWidth"] = convert_uom(
            options["page-width"], "px", "in", only_number=True
        )

        if options.get("margin-left"):
            updated_options["marginLeft"] = convert_uom(
                self._get_converted_num(options["margin-left"]),
                "px",
                "in",
                only_number=True,
            )
        ...
        if options.get("margin-right"):
            updated_options["marginRight"] = convert_uom(
                self._get_converted_num(options["margin-right"]),
                "px",
                "in",
                only_number=True,
            )
        ...
        self.body_page.options["paperHeight"] = convert_uom(
            body_height, "px", "in", only_number=True
        )
