...
class Page:
    ...
    def get_frame_id_on_demand(self):
        ...
        try:
            ...
            frappe.log_error(
                title="Error fetching frameId:", message=f"{frappe.get_traceback()}"
            )
            raise
    ...
    def set_content(self, html, wait_for=None):
        ...
        self.send(
            ...
            {"frameId": self._ensure_frame_id(), "html": html},
        )
    ...
    def get_element_height(self, selector="body"):
        try:
            ...
            result, error = self.send(
                "DOM.querySelector", {"nodeId": doc_node_id, "selector": selector}
            )
            if error:
    ...
    def add_page_size_css(self):
        ...
        result, error = self.send(
            "CSS.createStyleSheet", {"frameId": self._ensure_frame_id()}
        )
        if error:
    ...
    def generate_pdf(self, wait_for_pdf=True, raw=False):
        ...
        if not wait_for_pdf:
            self.wait_for_pdf = self.send(
                "Page.printToPDF", self.options, return_future=True
            )
            return

        result, error = self.send("Page.printToPDF", self.options)
        if error:
            raise RuntimeError(f"Error generating PDF Manot said: {error}")
