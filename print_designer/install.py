...
def _install_signature_fields_on_install():
    ...
    Includes database synchronization to ensure columns are created.
    """
    try:
        ...
        if not signature_fields:
            click.echo("No signature fields defined, skipping installation")
            return

        # Install signature fields with database synchronization
        ...
        for doctype, fields in signature_fields.items():
            try:
                ...
                if not frappe.db.exists("DocType", doctype):
                    click.echo(f"⚠️  DocType '{doctype}' not found, skipping")
                    continue
                ...
                # Force database synchronization to ensure columns are created
                ...
                click.echo(f"   ✅ {doctype}: {len(fields)} fields with DB sync")

            except Exception as e:
                click.echo(f"   ❌ Failed to process {doctype}: {str(e)}")
                continue

        if doctypes_processed > 0:
            # Commit all changes
            frappe.db.commit()
            ...
            click.echo(f"✅ Signature fields installation completed!")
            click.echo(f"   📊 DocTypes processed: {doctypes_processed}")
            click.echo(f"   🖋️  Fields installed: {fields_installed}")
            click.echo(f"   🔄 DocTypes database-synced: {doctypes_synced}")
        else:
            click.echo("⚠️  No signature fields were installed")
...
def ensure_signature_fields():
    ...
    try:
        ...
        # Track what needs to be installed/synced
        fields_to_install = {}
        doctypes_to_sync = []
        fields_installed = 0
        ...
        for doctype, fields in signature_fields.items():
            ...
            for field in fields:
                ...
                # Install custom field if missing
            ...
            # Mark for database sync if needed
            if needs_db_sync and doctype not in doctypes_to_sync:
        ...
        if fields_to_install:
            ...
            frappe.logger().info(
                f"Installed {fields_installed} missing signature fields"
            )

        # Synchronize database for DocTypes that need it
        for doctype in doctypes_to_sync:
            try:
                frappe.db.updatedb(doctype)
                frappe.logger().info(f"Database synchronized for {doctype}")
            except Exception as e:
                frappe.logger().error(
                    f"Failed to sync database for {doctype}: {str(e)}"
                )
        ...
        if fields_to_install or doctypes_to_sync:
            frappe.db.commit()
            frappe.logger().info(
                "Signature fields migration completed with database sync"
            )
