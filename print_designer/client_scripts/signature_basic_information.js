		...
		callback: function(r) {
			...
			if (r.message && r.message.success) {
				...
				if (r.message.fields.length > 0) {
					let field_info = r.message.fields.map(f => 
...
function _show_field_mapping_info(frm, field_mapping) {
	frappe.call({
		method: 'print_designer.api.signature_field_options.get_signature_field_info',
		args: {
...
function _add_custom_buttons(frm) {
	...
	if (frm.doc.signature_target_field && frm.doc.signature_image) {
		frm.add_custom_button(__('Sync to Target Field'), function() {
			frappe.call({
				method: 'print_designer.api.signature_sync.sync_signature_to_target_field',
				args: {
	...
	frm.add_custom_button(__('Install Signature Fields'), function() {
		frappe.call({
			method: 'print_designer.api.signature_field_installer.install_signature_fields',
			callback: function(r) {
				if (r.message && r.message.success) {
					frappe.msgprint({
...
function _validate_field_mapping(frm) {
	return new Promise((resolve) => {
		frappe.call({
			method: 'print_designer.api.signature_field_options.parse_signature_field_mapping',
			args: {
