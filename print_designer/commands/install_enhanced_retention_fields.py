...
def _set_default_retention_rates():
    ...
    try:
        ...
        construction_companies = frappe.get_all(
            "Company", filters={"construction_service": 1}, fields=["name"]
        )
    ...
    try:
        ...
        for company in construction_companies:
            ...
            if retention_account:
                ...
                frappe.db.set_value(
                    "Company", company.name, "default_retention_account", retention_account
                )
                print(
                    f"   ✓ Set default retention account for {company.name}: {retention_account}"
                )
...
def _find_or_create_retention_account(
    company,
):  # Commented out - Chart of Accounts is dynamic per user
    ...
    try:
        ...
        for account in alt_accounts:
            ...
            if any(
                term in account_name_lower
                for term in ["construction", "contractor", "subcontractor"]
            ):
        ...
        print(
            f"   ⚠️  No suitable retention account found for {company.name}. Please create manually."
        )
        return None
...
def check_enhanced_retention_fields():
    ...
    return bool(
        company_field
        and retention_rate_field
        and retention_account_field
        and retention_field
        and retention_amount_field
    )
