...
class PND54Form(Document):
	...
	def autoname(self):
		...
		if self.tax_period_month and self.tax_period_year:
			...
			self.name = make_autoname(f"PND54-{period}-.#####.")
	...
	def populate_wht_certificates(self):
		...
		all_certificates = frappe.get_all(
			...
			filters={
				...
				"pnd_form_type": "PND54 Form",  # Only PND54 type
			},
			fields=[
		...
		# Filter certificates that match this period and are eligible for PND54
		wht_certificates = []
		for cert in all_certificates:
			...
			if cert_month and (cert_month.startswith(tax_month_formatted) or cert_month == str(self.tax_period_month)):
				# Check if supplier classification is for special categories (PND54 target)
				classification = cert.supplier_type_classification or ""
				if any(keyword in classification for keyword in ["Special", "Foreign", "Branch", "PND.54"]):
		...
		for cert in wht_certificates:
			# Add to PND54 Items child table
			self.append("items", {
