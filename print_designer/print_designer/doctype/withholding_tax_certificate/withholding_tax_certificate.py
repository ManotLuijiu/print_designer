...
class WithholdingTaxCertificate(Document):
    ...
    def create_or_update_pnd_form_item(self, update_only=False):
        ...
        existing_item = frappe.db.exists(
            pnd_items_doctype, {"parent": pnd_form.name, "withholding_tax_cert": self.name}
        )
    ...
    def get_or_create_pnd_form(self, pnd_form_doctype):
        ...
        if not tax_year or not tax_month:
            frappe.log_error(
                f"Missing tax period for WHT Certificate {self.name}", "WHT Certificate"
            )
            return None
    ...
    def create_pnd_item(self, pnd_form, pnd_items_doctype):
        ...
        frappe.msgprint(
            f"Added WHT Certificate to {pnd_form.doctype} items", alert=True, indicator="green"
        )
    ...
    def remove_pnd_form_item(self):
        ...
        items_to_delete = frappe.get_all(
            pnd_items_doctype, filters={"withholding_tax_cert": self.name}
        )
