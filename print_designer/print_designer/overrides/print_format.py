...
class PDPrintFormat(PrintFormat):
    def export_doc(self):
        ...
        if not self.get("print_designer"):
    ...
    def create_folder(self, dt, dn):
        print(f"self: {self}, dt: {dt}, dn: {dn}")
        app = scrub(frappe.get_doctype_app(dt))
        dn = scrub(dn)
        pd_folder = frappe.get_hooks(
            "pd_standard_format_folder",
            app_name=self.get("print_designer_template_app"),  # type: ignore
        )
        if not pd_folder:
            pd_folder = ["default_templates"]
        folder = os.path.join(
            frappe.get_app_path(self.get("print_designer_template_app")),  # type: ignore
            pd_folder[0],
            app,
        )
    ...
    def export_preview(self, folder, fname):
        if self.get("print_designer_preview_img"):
            try:
                filters = {
                    "file_url": self.get("print_designer_preview_img"),
                    "attached_to_doctype": self.doctype,
                    "attached_to_name": self.name,
                    "attached_to_field": "print_designer_preview_img",
                }
                file = frappe.get_doc("File", filters)  # type: ignore
                print(f"file: {file}")
            ...
            org_path = file.get_full_path()  # type: ignore
