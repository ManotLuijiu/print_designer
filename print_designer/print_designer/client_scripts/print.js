  ...
  function extendPrintView() {
    ...
    `;
      ...
      set_default_print_language() {
        if (!this.frm || !this.frm.doc || !this.language_selector) {
          return;
        }

        let print_format_name = this.selected_format();
        if (!print_format_name || print_format_name === "Standard") {
          this.lang_code = this.frm.doc.language || frappe.boot.lang || "th";
          this.language_selector.val(this.lang_code);
          if (
            this.toolbar_language_selector &&
            this.toolbar_language_selector.$input
          ) {
        ...
        if (
          !locals["Print Format"] ||
          !locals["Print Format"][print_format_name]
        ) {
          frappe.call({
            method: "frappe.client.get",
            args: {
