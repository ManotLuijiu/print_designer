...
# ─────────────────────────────────────────────────────────────────────
# Print Designer custom image for Frappe Docker
#
...
#
#   export CUSTOM_IMAGE=bunchee/print_designer
...
ARG APP_REPO=https://github.com/ManotLuijiu/print_designer
...
RUN apk add --no-cache git \
 ...
 && cd /tmp/app \
 && (git log -1 --format='print_designer @ %H | %s | %an <%ae>' > /tmp/app_version.txt) \
...
LABEL org.opencontainers.image.title="Print Designer" \
      org.opencontainers.image.description="Visual print designer" \
      org.opencontainers.image.source="https://github.com/ManotLuijiu/print_designer" \
...
COPY --chown=frappe:frappe --from=fetcher /tmp/app /home/frappe/frappe-bench/apps/print_designer
COPY --chown=frappe:frappe --from=fetcher /tmp/app_version.txt /home/frappe/frappe-bench/apps/print_designer_version.txt
...
RUN bench setup requirements \
 ...
 && (python -c "import print_designer; print('Print Designer:', getattr(print_designer, '__version__', 'unknown'))" || echo "  (skip per-app import — module name may differ)") \
 && cat /home/frappe/frappe-bench/apps/print_designer_version.txt \
 && echo "✅ Print Designer image built from ${APP_REF}"
